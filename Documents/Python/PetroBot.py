...
**!@add** - добавляет видео в список (не более одного видео за раз) данного канала
...
async def on_message(message):
	...
	if message.content.startswith('!@help'):
		...
		if not os.path.isfile('list_file.txt'):
			list_file_names = []
			print('Файл ' + 'list_file.txt' + ' создан')
		else:
			with open ('list_file.txt', 'r') as list_file:
				list_file_names = json.load(list_file)
		list_file_names.append(message.content[5:].strip()) #.split('\n')
		await client.add_reaction(message, '✅') #White check mark
		print('Добавлено видео ' + message.content[5:].strip())

	elif message.content.startswith('!@show'):
		if not os.path.isfile(str(file_name)):
			await client.send_message(message.channel, 'список видео данного канала пуст')
			return
		...
		print('Показан список')

	elif message.content.startswith('!@poll'):
		await client.add_reaction(message, '✅') #White check mark
		await client.add_reaction(message, '❌') #Cross Mark
		print('Устроено голосование за видео' + message.content[5:].strip())
